...
"""
support features:
done: 1. in-batch and nce style loss: controlled by data_args
done: 2. multi-label retrieval
done: 3. hard-negative mining resuming from saved optimizer states
4. question: from the trainer class I found has_labels is always False which 
means it will never return eval_loss, but I remember the tensorboard logged 
the eval_loss before, I'll not modify the prediction_step for now, but if I 
found it actually won't return eval_loss, I should modify the prediction_step
...
class DenseTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        if self.args.multi_label:
            query, passage, target = inputs
            outputs = model(query=query, passage=passage, target=target)
        else:
...
def split_dense_inputs(model_input: dict, chunk_size: int):
    ...
    chunked_arg_val = [dict(zip(kk, tt)) for kk, tt in
                       zip(repeat(keys), zip(*chunked_tensors))]
