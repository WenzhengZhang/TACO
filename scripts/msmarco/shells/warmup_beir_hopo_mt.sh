...
MODEL_DIR=$TACO_DIR"/model/warmup_mt/mt_msmarco_beir_hopo/"
DATA_DIR=$TACO_DIR"/data/"
#RAW_DIR=$DATA_DIR"/raw/"
#PROCESSED_DIR=$DATA_DIR"/processed/bm25/"
LOG_DIR=$TACO_DIR"/logs/warmup_mt/mt_msmarco_beir_hopo/"
EMBEDDING_DIR=$TACO_DIR"/embeddings/warmup_mt/mt_msmarco_beir_hopo/"
RESULT_DIR=$TACO_DIR"/results/warmup_mt/mt_msmarco_beir_hopo/"
EVAL_DIR=$TACO_DIR"/metrics/trec/trec_eval-9.0.7/trec_eval-9.0.7/"
#ANCE_PROCESSED_DIR=$DATA_DIR"ance_hn_mt/mt_msmarco_beir_hopo/processed/naive/"
ANCE_MODEL_DIR=$TACO_DIR"/model/ance_hn_mt/mt_msmarco_beir_hopo/naive/"
mkdir -p $TACO_DIR
mkdir -p $PLM_DIR
mkdir -p $MODEL_DIR
mkdir -p $DATA_DIR
...
#    --resume_from_checkpoint $MODEL_DIR/checkpoint-28006
...
do
  ...
  if [ ${mt_set} == fever ] || [ ${mt_set} == hopo ]; then
    RAW_DIR=$DATA_DIR/beir/${mt_set}/raw/
    PROCESSED_DIR=$DATA_DIR/beir/${mt_set}/processed/bm25/
    ANCE_PROCESSED_DIR=$DATA_DIR/beir/${mt_set}/processed/ance_hn_mt/mt_msmarco_beir_hopo/naive/hn_iter_0/
  else
    RAW_DIR=$DATA_DIR/${mt_set}/raw/
    PROCESSED_DIR=$DATA_DIR/${mt_set}/processed/bm25/
    ANCE_PROCESSED_DIR=$DATA_DIR/${mt_set}/processed/ance_hn_mt/mt_msmarco_beir_hopo/naive/hn_iter_0/
  fi
  mkdir -p $ANCE_PROCESSED_DIR
  if [ ${mt_set} == zeshel ]; then
...
#  if [ ${mt_set} != msmarco ]; then
  ...
  if [ ${mt_set} == msmarco ]; then
    ...
    shuf -n 150000 $RAW_DIR/train.query.txt > $ANCE_PROCESSED_DIR/train.query.txt
