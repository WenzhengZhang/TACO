...
MODEL_DIR=$TACO_DIR"/model/ance_hn_mt/mt_msmarco_beir_hopo/"$mt_method
DATA_DIR=$TACO_DIR"/data/"
#RAW_DIR=$DATA_DIR"/raw/"
#PROCESSED_DIR=$DATA_DIR"/processed/bm25/"
LOG_DIR=$TACO_DIR"/logs/ance_hn_mt/mt_msmarco_beir_hopo/"$mt_method
EMBEDDING_DIR=$TACO_DIR"/embeddings/ance_hn_mt/mt_msmarco_beir_hopo/"$mt_method
RESULT_DIR=$TACO_DIR"/results/ance_hn_mt/mt_msmarco_beir_hopo/"$mt_method
EVAL_DIR=$TACO_DIR"/metrics/trec/trec_eval-9.0.7/trec_eval-9.0.7/"
#PROCESSED_DIR=$DATA_DIR"ance_hn_mt/mt_msmarco_beir_hopo/processed/"$mt_method
if [ -d $MODEL_DIR ]; then
  echo "$MODEL_DIR is not empty"
else
  echo "get initial model"
  mkdir -p $TACO_DIR"/model/ance_hn_mt/mt_msmarco_beir_hopo/"
  cp -r $TACO_DIR"/model/ance_hn_mt/mt_msmarco_beir_hopo/naive/" $MODEL_DIR
fi
#if [ -d $PROCESSED_DIR ]; then
#  echo "$PROCESSED_DIR is not empty"
#else
#  echo "get initial processed data"
#  mkdir -p $DATA_DIR"ance_hn_mt/mt_msmarco_beir_hopo/processed/"
#  cp -r $DATA_DIR"ance_hn_mt/mt_msmarco_beir_hopo/processed/naive/"  $PROCESSED_DIR
#fi
...
mt_sets=(msmarco zeshel fever hopo)
...
do
  ...
  do
    ...
    if [ ${mt_set} == fever ] || [ ${mt_set} == hopo ]; then
      RAW_DIR=$DATA_DIR/beir/${mt_set}/raw/
      NAIVE_INIT_DIR=$DATA_DIR/beir/${mt_set}/processed/ance_hn_mt/mt_msmarco_beir_hopo/naive/hn_iter_0
      PREFIX_PROCESSED=$DATA_DIR/beir/${mt_set}/processed/ance_hn_mt/mt_msmarco_beir_hopo/${mt_method}/
      PROCESSED_DIR=$PREFIX_PROCESSED/hn_iter_${hn_iter}/
    else
      RAW_DIR=$DATA_DIR/${mt_set}/raw/
      PREFIX_PROCESSED=$DATA_DIR/${mt_set}/processed/ance_hn_mt/mt_msmarco_beir_hopo/${mt_method}/
      NAIVE_INIT_DIR=$DATA_DIR/${mt_set}/processed/ance_hn_mt/mt_msmarco_beir_hopo/naive/hn_iter_0
      PROCESSED_DIR=$PREFIX_PROCESSED/hn_iter_${hn_iter}/
    fi
    if [ ${mt_set} == zeshel ]; then
...
#  cp $ckpt_dir/pytorch_model.bin $MODEL_DIR/pytorch_model.bin
  ...
  do
    ...
    if [ ${mt_set} == fever ] || [ ${mt_set} == hopo ]; then
      RAW_DIR=$DATA_DIR/beir/${mt_set}/raw/
      PROCESSED_DIR=$DATA_DIR/beir/${mt_set}/processed/ance_hn_mt/mt_msmarco_beir_hopo/${mt_method}/hn_iter_${new_hn_iter}/
    else
      RAW_DIR=$DATA_DIR/${mt_set}/raw/
      PROCESSED_DIR=$DATA_DIR/${mt_set}/processed/ance_hn_mt/mt_msmarco_beir_hopo/${mt_method}/hn_iter_${new_hn_iter}/
    fi
    mkdir -p $PROCESSED_DIR
    if [ ${mt_set} == zeshel ]; then
      dev_corpus_path=$RAW_DIR/psg_corpus_dev.tsv
...
#        --use_gpu
    ...
    if [ ${hn_iter} != ${last_hn_iter} ]; then
      ...
      if [ ${mt_set} == msmarco ]; then
        ...
        shuf -n 150000 $RAW_DIR/train.query.txt > $PROCESSED_DIR/train.query.txt
