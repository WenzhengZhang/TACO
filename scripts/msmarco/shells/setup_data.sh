...
echo "convert dev.tsv to dev.trec"
python $CODE_DIR/scripts/msmarco/convert_msmarco_to_trec_qrels.py \
    --input $RAW_DIR/dev.qrel.tsv \
    --output $RAW_DIR/dev.qrel.trec
...
if [ -f "$PROCESSED_DIR/train.jsonl" ]; then
    ...
    mv train_all.jsonl train.jsonl
#    export RANDOM=42
