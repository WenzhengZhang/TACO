...
#      rm $RESULT_DIR/${mt_set}/hn_iter_${hn_iter}/train.trec
      ...
      if [ ${mt_set} == zeshel ]; then
        ...
        if [ $hn_iter != 5 ] || [ ${mt_set} == nq ] || [ ${mt_set} == fever ]; then
  ...
  do
    ...
    if [ ${mt_set} == nq ]; then
      RAW_DIR=$DATA_DIR/kilt/${mt_set}/raw/
      PROCESSED_DIR=$DATA_DIR/kilt/${mt_set}/processed/ance_mt/mt_msmarco/${mt_method}/hn_iter_${new_hn_iter}/
    elif [ ${mt_set} == fever ]; then
      RAW_DIR=$DATA_DIR/beir/${mt_set}/raw/
      PROCESSED_DIR=$DATA_DIR/beir/${mt_set}/processed/ance_mt/mt_msmarco/${mt_method}/hn_iter_${new_hn_iter}/
    else
      RAW_DIR=$DATA_DIR/${mt_set}/raw/
      PROCESSED_DIR=$DATA_DIR/${mt_set}/processed/ance_mt/mt_msmarco/${mt_method}/hn_iter_${new_hn_iter}/
    fi
    mkdir -p $PROCESSED_DIR
    if [ ${mt_set} == zeshel ]; then
      dev_corpus_path=$RAW_DIR/psg_corpus_dev.tsv
